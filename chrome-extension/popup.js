document.addEventListener("DOMContentLoaded", function () {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      .addEventListener("click", async function () {
        ...
        await getUsername()
          ...
          .catch((error) => {
            ...
            document.getElementById("center-text-error").className =
              "center-text";
            document.getElementById("center-text-error").innerHTML =
              "Sign in and refresh the extension to get started!";
          });

        // Send the username to the background script
        chrome.runtime.sendMessage(
          { username: username, self: true },
          async (response) => {
            ...
            try {
              ...
              if (postData.famous) {
                document.getElementById("loading").className = "hidden";
                document.getElementById("center-text").innerHTML =
                  "Wow! This user is too famous for us to process";
                document.getElementById("center-text-error").className =
                  "center-text";
                document.getElementById("center-text-error").innerHTML =
                  "unfollowed.lol can only make a limited number of queries, sorry!";
                throw new Error(
                  "This user is too famous for us to process. Please try a different user."
                );
              ...
              link.addEventListener("click", function () {
                window.open(
                  `https://www.unfollowed.lol/user/${username}/`,
                  "_blank"
                );
              });
              ...
              document.getElementById("unfollowers").innerHTML =
                (Math.round(
                  (postData.unfollowers.length / postData.followings.length) *
                    100
                ) || 0) + "%";
              document.getElementById("fans").innerHTML =
                (Math.round(
                  (postData.fans.length / postData.followers.length) * 100
                ) || 0) + "%";
          ...
          chrome.scripting.executeScript(
            {
              ...
              function: () => {
                ...
                for (const element of profileElements) {
                  if (
                    element.href &&
                    element.textContent.trim() === "Profile"
                  ) {
                    const username = element.href.match(
    ...
    // FIXME
    if (
      localStorage.getItem("lastProcessed") &&
      localStorage.getItem("lastProcessed") > Date.now() - 1000 * 60 * 10 &&
      false
    ) {
      document.getElementById("goto").className = "hidden";
      document.getElementById("analysis").className = "hidden";
      ...
      const total_this_week = JSON.parse(
        localStorage.getItem("total_this_week")
      );
      ...
      if (total_this_week == 0 || total_today == 0) {
        document.getElementById("unfollowers").innerHTML =
          (Math.round((unfollowers.length / followings.length) * 100) || 0) +
          "%";
        document.getElementById("fans").innerHTML =
          (Math.round((fans.length / followers.length) * 100) || 0) + "%";
        document.getElementById("nodata-profile").appendChild(link);
        document.body.className = "profile-body";
        document.getElementById("nouser").className = "hidden";
        document.getElementById("nodata-profile").className = "profile";
